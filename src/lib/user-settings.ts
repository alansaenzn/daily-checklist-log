...
const userSettingsBaseSchema = z.object({
    ...
    focusTimerMinutes: z.number().int().min(1).max(120),
  });
...
// Momentum threshold utilities (legacy setting used by heatmaps)
...
type UserSettingsRow = {
  ...
  focus_timer_minutes: number | null;
};
...
function mapRowToSettings(row: UserSettingsRow): Partial<UserSettings> {
  return {
    ...
    focusTimerMinutes: row.focus_timer_minutes ?? DEFAULT_USER_SETTINGS.focusTimerMinutes,
  };
}
...
function mapSettingsPatchToRow(patch: UserSettingsUpdate) {
  ...
  if ("focusTimerMinutes" in patch) mapped.focus_timer_minutes = patch.focusTimerMinutes ?? DEFAULT_USER_SETTINGS.focusTimerMinutes;
  return mapped;
}

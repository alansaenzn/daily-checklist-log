...
/**
 * Fetch ALL active, non-archived tasks for the current user (across all projects).
 * Useful for getting all categories and allowing cross-project task viewing.
 */
export async function getAllActiveTasks(): Promise<TaskData[]> {
  const supabase = supabaseServer();
  const { data: userData, error: userErr } = await supabase.auth.getUser();
  if (userErr || !userData.user) throw new Error("Not authenticated");
